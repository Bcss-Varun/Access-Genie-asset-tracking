...
export default function PartDetailPage({ params }: { params: Promise<{ sku: string }> }) {
    ...
    return (
      <div className="h-full flex flex-col space-y-6">
        <EmptyState
          ...
          action={<Link href="/inventory"><Button variant="outline">← Back to Inventory</Button></Link>}
        />
      </div>
    );
  }
  ...
  const facts: { label: string; value: React.ReactNode }[] = [
    ...
    { label: 'Category', value: part.category },
    {
      label: 'Warehouse',
      value: (
        <Link href={`/warehouses/${part.warehouseId}`} className="text-primary-600 hover:text-primary-700 font-medium">
    ...
    },
    {
      label: 'Supplier',
      value: (
        <Link href={`/suppliers/${part.supplierId}`} className="text-primary-600 hover:text-primary-700 font-medium">
