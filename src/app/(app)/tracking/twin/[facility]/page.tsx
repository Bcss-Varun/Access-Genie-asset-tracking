...
import type { AssetPresence, TrackedFacility, ZoneKind } from '@/types/tracking';
...
export default function DigitalTwinPage({ params }: { params: Promise<{ facility: string }> }) {
    ...
    <div className="space-y-4 pb-4">
        ...
        <aside className="space-y-4">
            ...
            >
              ...
              <div className="mt-4">
                ...
                {selected.assets.length > 6 && (
                  <Link
                    href={`/tracking/geofences?zone=${selected.zone.id}&${scoped}`}
                ...
                {selected.infra.length === 0 ? (
                  ...
                  <ul className="mt-2 space-y-1.5">
                    {selected.infra.map((d) => (
                      <li key={d.id}>
                        <Link
                          href={`/tracking/infrastructure?tab=tags&device=${d.id}&${scoped}`}
              ...
              <div className="mt-4 flex flex-wrap gap-2 border-t border-slate-100 pt-3">
                <Link href={`/tracking/geofences?zone=${selected.zone.id}&${scoped}`}>
                  <Button size="sm">Locate in this zone</Button>
                </Link>
                {selected.room && (
                  <Link href={`/tracking/inventory?tab=rooms&room=${selected.room.id}&${scoped}`}>
          ...
          <Panel title="Work this building" subtitle="Every module, already scoped here" padded={false}>
            <ul className="divide-y divide-slate-100">
              {MODULE_LINKS.map((m) => (
                <li key={m.href}>
                  <Link href={m.href} className="flex items-center gap-3 px-4 py-2.5 transition-colors hover:bg-slate-50">
      ...
      <Drawer
        ...
        footer={
          asset && (
            <>
              <Link href={`/tracking?asset=${asset.assetId}&${scoped}`}>
                <Button size="sm">Open in Locate Assets</Button>
              </Link>
              {asset.alertIds.length > 0 && (
                <Link href={`/tracking/alerts?tab=queue&alert=${asset.alertIds[0]}`}>
