...
// Every action mutates state in-session, so acknowledging an alert visibly
// moves it out of the New bucket and every count above it changes with it.
//
...
import { DEMO_NOW, cn, formatMoney, relTime } from '@/lib/utils';
...
const ACTION_STATE: Record<AlertAction, AlertLifecycle> = {
...
const DAY_START = DEMO_NOW - 9 * 3_600_000;
const NOW_ISO = new Date(DEMO_NOW).toISOString();
...
const slaMs = (a: TrackingAlert) => Date.parse(a.slaDueAt) - DEMO_NOW;
...
export default function TrackingAlertsPage() {
  ...
  // ── Lifecycle actions — all local, all visible immediately ─────────────────

  const act = (ids: string[], action: AlertAction) => {
    ...
    toast({
      title: `${targets.length} alert${targets.length > 1 ? 's' : ''} ${state.toLowerCase()}`,
      description: action === 'assign' ? `Now owned by ${actor}` : ids4 + (targets.length > 4 ? ` +${targets.length - 4} more` : ''),
      tone: action === 'resolve' ? 'success' : action === 'escalate' ? 'error' : 'info',
    });
  };

  /** Several alerts that are really one event become an incident with a commander. */
  const createIncidentFromSelection = () => {
    ...
    const id = `INC-45${String(createdIncidents.length + 1).padStart(2, '0')}`;
    const incident: Incident = {
      id,
      title: `${picked[0].category} — ${picked.length} linked alert${picked.length > 1 ? 's' : ''}`,
      severity: rank === 0 ? 'Sev1' : rank === 1 ? 'Sev2' : 'Sev3',
      state: 'Open',
      ...
      assetsAffected: new Set(picked.flatMap((a) => (a.assetId ? [a.assetId] : []))).size,
      valueAtRiskInr: picked.reduce((s, a) => s + (a.valueAtRiskInr ?? 0), 0),
      nextAction: 'Confirm containment, then work each linked alert to resolution',
    };
    setCreatedIncidents((prev) => [incident, ...prev]);
    setPatches((prev) => {
      const next = { ...prev };
      for (const a of picked) next[a.id] = { ...next[a.id], incidentId: id };
      return next;
    });
    setSelected(new Set());
    setTab('incidents');
    ...
    toast({
      title: `${id} opened`,
      description: `${picked.length} alerts linked · ${incident.severity} · commander ${session.user.name}`
        + (alreadyLinked ? ` · ${alreadyLinked} left on their existing incident` : ''),
      tone: 'success',
    });
  };

  const moveIncident = (id: string, state: Incident['state']) => {
    setIncidentStates((prev) => ({ ...prev, [id]: state }));
    toast({
      title: `${id} marked ${state.toLowerCase()}`,
      description: state === 'Contained' ? 'Spread stopped — linked alerts stay open until each is worked' : 'Narrative closed for this event',
      tone: state === 'Closed' || state === 'Resolved' ? 'success' : 'info',
    });
  };
  ...
  const toggleRule = (r: AutomationRule) => {
    ...
    toast({
      title: next ? `${r.name} enabled` : `${r.name} paused`,
      description: next ? `Matches now raise ${r.priority} to ${r.assignTeam}` : 'Matching signals will stop raising alerts',
  ...
  const elapsed = scoped.filter((a) => Date.parse(a.slaDueAt) <= DEMO_NOW);
    ...
    <div className="space-y-5 pb-4">
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6">
        <StatTile
          ...
          sub={cleared > 0 ? `${cleared} cleared this session` : `of ${scoped.length} raised in scope`}
            ...
            <div className="flex flex-wrap items-center gap-2 border-b border-primary-100 bg-primary-50 px-4 py-2 text-sm">
              ...
              <div className="flex flex-wrap items-center gap-1">
                {(Object.keys(ACTION_LABEL) as AlertAction[]).map((a) => (
                  <button key={a} type="button" onClick={() => act(Array.from(selected), a)}
                ...
                <button type="button" onClick={createIncidentFromSelection}
              ...
              {automationRules.map((r) => {
                ...
                return (
                  <tr key={r.id} onClick={() => setOpenRuleId(r.id)}
                    ...
                    <td className={cn(td, 'text-right')} onClick={(e) => e.stopPropagation()}>
                      <button
                        type="button" role="switch" aria-checked={on} onClick={() => toggleRule(r)}
      ...
      <Drawer
        ...
        footer={drawerAlert && (
          <>
            {(Object.keys(ACTION_LABEL) as AlertAction[]).map((a) => (
              <Button key={a} size="sm" variant={a === 'resolve' ? 'primary' : 'outline'} onClick={() => act([drawerAlert.id], a)}>
                {ACTION_LABEL[a]}
              </Button>
            ))}
          ...
          <div className="space-y-5">
            ...
            <dl className="divide-y divide-slate-100 border-y border-slate-100">
              {drawerAlert.assetName && (
                <Field label="Asset">
                  {drawerAlert.assetId ? (
                    <Link href={`/tracking?asset=${drawerAlert.assetId}`} className="font-medium text-primary-600 hover:text-primary-700">
                      {drawerAlert.assetName} →
                    </Link>
                  ) : drawerAlert.assetName}
                </Field>
              ...
              {drawerAlert.deviceId && (
                <Field label="Device">
                  <Link href={`/tracking/infrastructure?tab=tags&device=${drawerAlert.deviceId}`} className="font-medium text-primary-600 hover:text-primary-700">
                    {drawerAlert.deviceId} →
                  </Link>
                </Field>
              )}
        ...
        footer={drawerIncident && (
          <>
            <Button
              ...
              onClick={() => moveIncident(drawerIncident.id, 'Contained')}
            >
              Contain
            </Button>
            <Button size="sm" variant="outline" disabled={incidentSettled} onClick={() => moveIncident(drawerIncident.id, 'Resolved')}>
              Resolve
            </Button>
            <Button size="sm" disabled={drawerIncident.state === 'Closed'} onClick={() => moveIncident(drawerIncident.id, 'Closed')}>
              Close incident
            </Button>
      ...
      <Drawer
        ...
        footer={drawerRule && (
          <>
            <Button size="sm" variant={ruleEnabled(drawerRule) ? 'outline' : 'primary'} onClick={() => toggleRule(drawerRule)}>
