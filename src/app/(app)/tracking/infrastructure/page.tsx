...
import { DEMO_NOW, cn, formatDate, relTime } from '@/lib/utils';
...
const PROVISION_STAMP = new Date(DEMO_NOW).toISOString();
const PAGE_SIZE = 12;
...
const daysUntil = (iso: string) => Math.round((Date.parse(iso) - DEMO_NOW) / 86_400_000);
...
export default function TrackingInfrastructurePage() {
  ...
  /* eslint-disable react-hooks/set-state-in-effect -- mount-only seed from the URL */
  useEffect(() => {
  ...
  // ── Actions — every one acknowledges itself, and where it changes something
  //    it changes it in session so the operator can see the effect. ───────────
  ...
  const reboot = (ids: string[]) => {
    setFlag(ids, { rebooting: true });
    toast({ title: 'Reboot queued', description: `${plural(ids.length)} will drop for roughly 40 seconds`, tone: 'info' });
  };
  const scheduleFirmware = (ids: string[]) => {
    setFlag(ids, { firmwareScheduled: true });
    toast({ title: 'Firmware scheduled', description: `${plural(ids.length)} added to the 01:00 window tonight`, tone: 'success' });
  };
  const markReplacement = (ids: string[]) => {
    setFlag(ids, { replacement: true });
    toast({ title: 'Marked for replacement', description: `${plural(ids.length)} added to the replacement queue`, tone: 'success' });
  };
  ...
  const bulk = (fn: (ids: string[]) => void) => { fn([...selected]); setSelected(new Set()); };
  ...
  const submitProvision = () => {
    ...
    const parent = form.parentId ? parentOptions.find((p) => p.id === form.parentId) : undefined;
    const device: TrackingDevice = {
      id: `DEV-NEW-${String(added.length + 1).padStart(2, '0')}`,
      name, role: form.role, state: 'Unprovisioned', technology: form.technology,
      facility: form.facility, zone,
      firmware: BASELINE_FIRMWARE[form.role], firmwareLatest: BASELINE_FIRMWARE[form.role],
      uptimePct: 0, serves: 0, parentId: form.parentId || undefined,
      lastSeen: PROVISION_STAMP, installedAt: PROVISION_STAMP,
      diagnostics: [
        { label: 'Provisioning', value: 'Awaiting first check-in', state: 'warn' },
        { label: 'Power', value: 'Not yet confirmed on site', state: 'warn' },
        { label: 'Uplink', value: parent ? `Will report through ${parent.name}` : 'Reports directly to the platform', state: 'ok' },
      ],
    };
          ...
          <Panel key={p.id} title={p.name} subtitle={`${p.role} · ${p.facility} ▸ ${p.zone}`}
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              {p.state === 'Offline' && zone && (
                <Link href={`/tracking/geofences?zone=${zone.id}`}>
  ...
  return (
    <div className="space-y-5 pb-4">
      ...
      {tab === 'readers' && (
        <div className="space-y-4">
          ...
          <Panel
            ...
            actions={<Link href={`/tracking/twin/${mapFacility.slug}`}><Button size="sm" variant="outline">🏢 Open Digital Twin</Button></Link>}
          >
            {scope === 'all' && (
              <div className="mb-3">
                ...
                <tbody className="divide-y divide-slate-100">
                  {cells.map((c) => (
                    <tr key={c.zoneId} className="transition-colors hover:bg-slate-50">
                      <td className={td}>
                        <Link href={`/tracking/geofences?zone=${c.zoneId}`} className="text-sm font-medium text-slate-900 hover:text-primary-600">{c.zone}</Link>
                    ...
                    <td className={cn(td, 'whitespace-nowrap text-right')}>
                      {c.state === 'Running' && (
                        <button type="button" className="rounded-md px-2 py-1 text-xs font-medium text-slate-600 hover:bg-slate-100"
                          onClick={() => { setCampaign(c.id, { state: 'Paused' }); toast({ title: `${c.name} paused`, description: 'In-flight devices will finish; the rest hold', tone: 'info' }); }}>
                          Pause
                        </button>
                      )}
                      {(c.state === 'Paused' || c.state === 'Scheduled') && (
                        <button type="button" className="rounded-md px-2 py-1 text-xs font-medium text-primary-700 hover:bg-primary-50"
                          onClick={() => { setCampaign(c.id, { state: 'Running' }); toast({ title: `${c.name} resumed`, description: 'Resuming from the last confirmed device', tone: 'success' }); }}>
                          Resume
                        </button>
                      )}
                      {c.failed > 0 && (
      ...
      <Drawer
        ...
        footer={open && (
          <>
            <Button size="sm" variant="outline" onClick={() => reboot([open.id])}>↻ Reboot</Button>
            <Button size="sm" variant="outline" onClick={() => runDiagnostics(open)}>🩺 Run diagnostics</Button>
            <Button size="sm" variant="outline" onClick={() => scheduleFirmware([open.id])}>⬆ Schedule firmware</Button>
            <Button size="sm" variant="danger" className="ml-auto" onClick={() => markReplacement([open.id])}>Mark for replacement</Button>
          </>
        )}
      >
        {open && (
              ...
              <dl className="mt-1 divide-y divide-slate-100">
                ...
                {open.assetId && (
                  <Field label="Bound asset">
                    <Link href={`/assets/${open.assetId}`} className="font-medium text-primary-600 hover:text-primary-700">{open.assetName ?? open.assetId}</Link>
                  </Field>
                )}
              </dl>
            </section>
                  ...
                  <span className="inline-flex items-center gap-2">
                    ...
                    {isBehind(open) ? (
                      <>
                        ...
                        <button type="button" onClick={() => scheduleFirmware([open.id])}
      ...
      <Drawer
        ...
        footer={
          <>
            ...
            <Button className="ml-auto" onClick={submitProvision} disabled={zoneOptions.length === 0}>Add to the estate</Button>
          </>
        }
      >
        <div className="space-y-4">
