...
export default function CustodyChainPage({ params }: { params: Promise<{ assetId: string }> }) {
      ...
      <div className="h-full flex flex-col space-y-6">
        ...
        <div className="glass-panel rounded-xl">
          <EmptyState
            ...
            action={<Link href="/custody"><Button variant="outline">← Back to Chain of Custody</Button></Link>}
          />
        </div>
      </div>
    );
  ...
  return (
    <div className="h-full flex flex-col space-y-6">
      <PageHeader
        ...
        breadcrumb={[{ label: 'Compliance' }, { label: 'Custody', href: '/custody' }, { label: assetName }]}
        actions={asset && (
          <Link href={`/assets/${assetId}`}><Button variant="outline">View asset profile →</Button></Link>
        )}
      />
