    ...
    <div className="h-full flex flex-col space-y-6">
      ...
      <Card className="p-0 overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-200 flex items-center justify-between">
          ...
          <Link href="/reorder" className="text-xs font-medium text-primary-600 hover:underline">Replenishment queue →</Link>
        </div>
              ...
              {belowReorder.map((p) => {
                ...
                return (
                  <tr key={p.id} className="hover:bg-slate-50 transition-colors">
                    <td className={td}>
                      <Link href={`/inventory/${p.sku}`} className="font-medium text-slate-900 hover:text-primary-600">{p.sku}</Link>
