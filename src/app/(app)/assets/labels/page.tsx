...
import {
  DEFAULT_TEMPLATE_ID, deviceById, deviceCanRun, identityStatus, printDevices, seedTemplates,
} from '@/lib/label-data';
...
import { KIND_FOR_MEDIUM, encodesTag } from '@/types/label';
import type { LabelFieldKey, LabelMedium } from '@/types/label';
...
export default function LabelPrintingPage() {
  ...
  const template = seedTemplates.find((t) => t.id === DEFAULT_TEMPLATE_ID) ?? seedTemplates[0];
                ...
                <tbody className="divide-y divide-slate-100">
                  {coverageRows.map(({ asset, status }) => (
                    <tr key={asset.id} className="transition-colors hover:bg-slate-50">
                      <td className={td}>
                        <Link href={`/assets/${asset.id}`} className="flex items-center gap-2.5 group">
