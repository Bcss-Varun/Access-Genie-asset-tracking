...
import type { AttributeDef, Criticality } from '@/types/asset';
import type { RegisteredAsset, RegistrationSeed } from '@/types/onboarding';
    ...
    <form onSubmit={submit} className="space-y-5">
            ...
            <div className="space-y-2">
              {dup.matches.map((m) => (
                <div key={m.id} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-slate-200 bg-white px-4 py-2.5">
                  ...
                  <Link href={`/assets/${m.id}`} className="shrink-0 text-xs font-medium text-primary-600 hover:underline">
                    Open existing →
                  </Link>
                </div>
              ))}
      ...
      <div className="flex flex-col-reverse items-stretch gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center justify-end gap-2">
          <Button type="button" variant="outline" onClick={onBack}>← Change source</Button>
          <Button type="submit" disabled={touched && !canCommit}>Create asset →</Button>
        </div>
      </div>
    </form>
  );
