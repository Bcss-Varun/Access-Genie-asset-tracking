...
import { mockUsers } from '@/lib/rbac';
import { cn } from '@/lib/utils';
import type { GateResult, RegisteredAsset } from '@/types/onboarding';
...
export function PlaceCard({
  ...
  const owners = useMemo(() => {
    const teams = Array.from(new Set(mockAssets.map((a) => a.custodian))).filter((c) => /team|ops/i.test(c));
    return [...teams, ...mockUsers.map((u) => u.name)];
  }, []);
