...
export const SCOPE_STORAGE_KEY = 'ag-tracking-scope';

/**
 * The URL and sessionStorage are client-only stores, so they are read through
 * `useSyncExternalStore` rather than an effect: the server snapshot is always
 * the stable default, React swaps in the real value as part of hydration, and
 * no cascading re-render is triggered on the way. Nothing ever notifies — the
 * value only changes when this hook itself changes it — so `subscribe` is inert.
 */
const inertStore = () => () => {};
...
function readScope(): string {
  ...
  const next = url ?? stored;
  return next && (next === 'all' || TRACKED_FACILITIES.some((f) => f.slug === next)) ? next : 'all';
}

/**
 * The facility scope, shared across the modules through sessionStorage so that
 * jumping Dashboard → Locate Assets keeps you in the same building.
 */
export function useFacilityScope(): [string, (slug: string) => void] {
  const restored = useSyncExternalStore(inertStore, readScope, () => 'all');
  const [chosen, setChosen] = useState<string | null>(null);
  const scope = chosen ?? restored;

  const update = useCallback((slug: string) => {
    setChosen(slug);
    window.sessionStorage.setItem(SCOPE_STORAGE_KEY, slug);
