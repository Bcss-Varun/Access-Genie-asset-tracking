...
export const queue = asyncHandler(async (req: Request, res: Response) => {
  ...
  sendData(res, await service.fieldQueue(requireScope(req), assignee));
});

/** Everything unassigned or assigned to anyone — the planner's view. */
export const allWork = asyncHandler(async (req: Request, res: Response) => {
  sendData(res, await service.fieldQueue(requireScope(req)));
});
...
export const scan = asyncHandler(async (req: Request, res: Response) => {
  ...
  sendData(res, await service.scanAsset(requireScope(req), req.params.id as string, actor, zone));
});
