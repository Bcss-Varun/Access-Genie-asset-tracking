...
export const upload = asyncHandler(async (req: Request, res: Response) => {
  ...
  const created = await service.uploadDocument(body, actorOf(req));
  recordAudit(req, {
...
export const download = asyncHandler(async (req: Request, res: Response) => {
  const { name, mimeType, body } = await service.documentContent(req.params.id as string);
...
export const remove = asyncHandler(async (req: Request, res: Response) => {
  ...
  await service.deleteDocument(id, actorOf(req));
