...
import type { AssetCategory, AttributeDef, Criticality, DocType, SensorKind } from '@access-genie/shared';
import type { GateKey } from '@access-genie/shared';
    ...
    return (
      <div className="flex h-full flex-col space-y-6">
        ...
        <EmptyState
          ...
          action={<Link to="/admin/classes"><Button variant="outline">Back to classes</Button></Link>}
        />
      </div>
    );
  }
    ...
    <div className="flex h-full flex-col space-y-6">
      <PageHeader
        ...
        actions={
          <>
            ...
            <Link to="/admin/classes"><Button variant="outline">Done</Button></Link>
          </>
        }
      />
      ...
      <Section title="Basics" blurb="What this class is called, and what belongs in it.">
        <div className="grid grid-cols-1 gap-x-5 gap-y-4 sm:grid-cols-2">
          ...
          <div>
            <label className={label} htmlFor="cls-category">Reports as</label>
            <select id="cls-category" className={input} value={cls.category} onChange={(e) => set('category', e.target.value as AssetCategory)}>
              {ASSET_CATEGORIES.map((c) => <option key={c} value={c}>{c}</option>)}
            </select>
            <p className="mt-1 text-xs text-slate-400">
              The category assets of this class are filed under across the registry filters, dashboards and charts.
              Changing it affects assets registered from here on.
            </p>
          </div>
        </div>
      </Section>
