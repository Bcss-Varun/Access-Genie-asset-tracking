...
import { allSensors } from '@/lib/dataset';
...
import type { SensorKind } from '@access-genie/shared';
import type { GateResult, RegisteredAsset, TagBinding } from '@access-genie/shared';
...
export function TrackCard({
  ...
  const conflict = useMemo(() => {
    ...
    const hit = allSensors.find((s) => s.tagId?.toLowerCase() === q && takenTags.has(s.tagId));
    if (!hit) return null;
