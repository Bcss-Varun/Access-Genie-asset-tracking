    ...
    return (
      <div className="h-full flex flex-col space-y-6">
        <EmptyState
          ...
          action={<Link to="/gateways"><Button variant="outline">← Back to Gateways</Button></Link>}
        />
      </div>
    );
  }
              ...
              <tbody className="divide-y divide-slate-100">
                {sensors.map((s) => (
                  <tr key={s.id} className="hover:bg-slate-50 transition-colors">
                    <td className={td}>
                      <Link to={`/sensors/${s.id}`} className="font-medium text-slate-900 hover:text-primary-600">{s.name}</Link>
                    ...
                    <td className={td}>
                      {s.assetId ? (
                        <Link to={`/assets/${s.assetId}`} className="text-primary-600 hover:text-primary-700 font-medium">
